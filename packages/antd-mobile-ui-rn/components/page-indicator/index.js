...
const PageIndicator = function (p) {
    ...
    for (let i = 0; i < props.total; i++) {
        items.push(<View key={i} className={classnames(`${classPrefix}-item`, props.direction === "vertical" ? `${classPrefix}-item-column` : null, {
    ...
    return withNativeProps(props, <View className={classnames(`${classPrefix}`, `${classPrefix}-${props.direction}`)}>
      {items}
    </View>);
};
